...
from mh_dave2_model import generateModel as generateModelDave2
from mh_autumn import generateModel as generateModelAutumn
from mh_chauffeur import generateModel as generateModelChauffeur
...
epochs = 50
batch_size = 64
init = 0  ##  The index to start saving the model from (when resuming training)...

def generateModel(image_shape, model='dave2'):
    if 'dave2' in model.lower():
        print('Generating dave2 model...')
        model = generateModelDave2(image_shape, dave2=dave2)
    elif 'autumn' in model.lower():
        print('Generating autumn model...')
        model = generateModelAutumn(image_shape)
    elif 'chauffeur' in model.lower():
        print('Generating chauffeur model...')
        model = generateModelChauffeur(image_shape)
    return model
...
def trainModel(datagen_train, datagen_val, X_train, y_train, X_val, y_val, X_test, y_test, model_name):
    model = generateModel(image_shape, model_name)
